...
const ZFS_TOOL: &[&str] = &["/usr/sbin/zfs", "/sbin/zfs", "/usr/local/sbin/zfs"];
const ZPOOL_TOOL: &[&str] = &["/usr/sbin/zpool", "/sbin/zpool", "/usr/local/sbin/zpool"];
...
pub fn detect(deadline: Instant) -> DiskEncryption {
    ...
    let swaps = fs::read_to_string(SWAPS).unwrap_or_default();
...
pub(crate) fn detect_at(
    ...
    let boot = volume_state(sysfs, zfs, &root_backing);
    let others = if boot == VolumeState::Encrypted {
        other_fixed_volume_states(sysfs, zfs, &mounts, swap_devnos, &root_backing, deadline)
    } else {
        Some(Vec::new())
    };
...
/// Inspects each mounted fixed volume and swap device once. Returns `None` if
...
fn other_fixed_volume_states(
    ...
    let mounted = mounts
        ...
        .filter_map(|mount| backing_of(sysfs, mount));
    let swapped = swap_devnos
        .iter()
        .filter_map(|devno| swap_backing(sysfs, mounts, devno));
    ...
    for backing in mounted.chain(swapped) {
        ...
        states.push(volume_state(sysfs, zfs, &backing));
    }
...
/// Resolves active swap areas to backing device numbers.
fn swap_devnos(swaps: &str) -> Vec<String> {
    parse_swaps(swaps)
        .iter()
        .filter_map(|path| devno_of(Path::new(path)))
        .collect()
}
...
fn volume_state(sysfs: &Sysfs, zfs: &ZfsTables, backing: &Backing) -> VolumeState {
    match backing {
        Backing::Devices(devnos) => combine(devnos.iter().map(|devno| device_state(sysfs, devno))),
        Backing::Zfs(dataset) => zfs_state(sysfs, zfs, dataset),
    }
}
...
fn devno_for_name(sys_dev_block: &Path, name: &str) -> Option<String> {
    fs::read_dir(sys_dev_block).ok()?.flatten().find_map(|entry| {
...
pub(crate) fn parse_zpool_status(text: &str) -> HashMap<String, Vec<String>> {
    ...
    for line in text.lines() {
        ...
        if path.starts_with('/') {
            pools.entry(pool.clone()).or_default().push(path.to_string());
        }
    }
...
fn zfs_state(sysfs: &Sysfs, zfs: &ZfsTables, dataset: &str) -> VolumeState {
    ...
    combine(vdevs.iter().map(|path| {
        match devno_for_name(sysfs.dev_block, &device_name(path)) {
            Some(devno) => device_state(sysfs, &devno),
...
fn device_state(sysfs: &Sysfs, devno: &str) -> VolumeState {
    match stack_is_encrypted(&sysfs.dev_block.join(devno), MAX_STACK_DEPTH) {
...
/// Returns true only if every path to physical storage crosses dm-crypt, and
/// `None` if any path cannot be inspected.
fn stack_is_encrypted(dir: &Path, depth: u32) -> Option<bool> {
    if depth == 0 {
    ...
    let slaves = match fs::read_dir(dir.join("slaves")) {
        ...
        Err(e) if e.kind() == ErrorKind::NotFound => return Some(false),
        Err(_) => return None,
    };

    let mut layers = Vec::new();
    for slave in slaves {
        let Ok(slave) = slave else { return None };
        layers.push(stack_is_encrypted(&slave.path(), depth - 1));
    }

    // Plaintext takes precedence; otherwise any unreadable leg makes this unknown.
    if layers.contains(&Some(false)) {
        Some(false)
    } else if layers.contains(&None) {
        None
    } else {
        Some(!layers.is_empty())
    }
}
...
99 1 253:1 / / rw,relatime shared:1 - ext4 /dev/mapper/vg-root rw";
    ...
    fn a_btrfs_root_missing_from_sysfs_falls_back_to_the_named_device() {
        ...
        assert_eq!(detect(&sysfs, BTRFS_ROOT).status.as_str(), "enabled");
    }
...
/swap\\040file                            file\t\t2097148\t\t0\t\t-3";
    ...
    fn swap_paths_are_resolved_to_the_device_holding_them() {
        ...
        assert_eq!(swap_devnos(&swaps), [devno_of(dir.path()).unwrap()]);
    }
    ...
    fn an_unencrypted_swap_partition_makes_the_machine_partial() {
        ...
        let mountinfo = "28 1 253:0 / / rw,relatime - ext4 /dev/mapper/root rw";
        let swap = vec!["8:4".to_string()];
        assert_eq!(
            detect_with(&sysfs, mountinfo, &swap, &ZfsTables::default())
                .status
                .as_str(),
            "partial"
        );
    ...
    #[test]
    fn a_sweep_that_runs_out_of_time_reports_unknown() {
        let sysfs = SysfsFixture::new();
        let crypt = sysfs.mapper("dm-0", Some("253:0"), "CRYPT-LUKS2-4f1c-root\n");
        sysfs.slave(&crypt, &sysfs.partition("sda", "sda3", "8:3"));
        sysfs.partition("sdb", "sdb1", "8:17");
