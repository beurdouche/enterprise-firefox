...
pub fn bytes_to_value(bytes: &[u8]) -> Result<Value, LockstoreKeystoreError> {
    use base64::Engine;
...
pub fn value_to_bytes(value: &Value) -> Result<Vec<u8>, LockstoreKeystoreError> {
    use base64::Engine;

    // Convert Value to variant to extract the string
    let variant = value.to_variant().map_err(|e| {
        LockstoreKeystoreError::Serialization(format!("Failed to convert to variant: {:?}", e))
    })?;
    ...
    unsafe { variant.GetAsAUTF8String(&mut *cstring) }
        .to_result()
        .map_err(|e| {
            LockstoreKeystoreError::Serialization(format!("Failed to get string: {:?}", e))
        })?;
    ...
    base64::engine::general_purpose::STANDARD
        ...
        .map_err(|e| {
            LockstoreKeystoreError::Serialization(format!("Failed to decode base64: {}", e))
        })
}
