...
pub fn detect(deadline: Instant) -> DiskEncryption {
    ...
    let others = if boot == VolumeState::Encrypted {
        other_volume_states(&volumes, &boot_guid, deadline)
    } else {
        Some(Vec::new())
    };
...
/// Enumerates mounted fixed volumes, or returns `None` if enumeration is incomplete.
fn fixed_volumes() -> Option<Vec<Volume>> {
...
fn is_fixed_drive(mount_path: &str) -> bool {
    unsafe { GetDriveTypeW(to_wide(mount_path).as_ptr()) == DRIVE_FIXED }
}
